...
\begin{itemize}
	\item Functions and Graphs, which is broken up primarily into families of functions, with some extra concepts thown into some generalist topics:
		\begin{itemize}	
			...
			\item Exponentials and Logarithms (MMu2t1, MMu4t1)
...
\begin{itemize}
	\item The alignment from MMu1t1 to S1M1 and S1M2 is quite good, with slight differences in terminology (\gls{ac} refers to inverse proportion while \gls{sace} refers to reciprocal for example) and focus (\gls{sace} puts much more of an emphasis on polynomials, seperating it into it's own topic (S1M2) and breaking it down into much more granular concepts).
	\item SMu3t2 and S2SM3 -- extended functions/ graphs/ etc.
\end{itemize}
