...
This thesis is concerned with the mathematics bridging courses offered at the \gls{uofa} through the \gls{mlc}: MathsStart and MathsTrack. These courses are intended to take students with very little mathematics background and bring them to a state where they can continue their education (which will generally have some mathematics pre-requisites or assumed knowledge) and be successful. The courses are structured to be student-paced, with no deadlines having been set apriori, in a deliberate attempt to both alleiviate test-anxiety, and to aclimitise students to a learning environment in which the primary onus for motivation is on the students. Although the bridging courses are offered through the \gls{mlc} at the \gls{uofa}, the cohort of students come from a very diverse set of backgrounds and have a variety of needs. Although the majority of students enrolling in these courses are planning to continue into tertiary education at the \gls{uofa}, many students do not intend to continue into tertiary education at all, but instead enroll in the bridging courses to meet the requisite assumed knowledge for other pathways --- pilot training in the defence forces, for example. The purpose, structure, and context for these courses are discussed in more detail below, but this brief introduction is enough to introduce the ``guiding question'', or ``purpose statement'' for this thesis, which is:

\begin{quote}
	How can MathsStart and MathsTrack be improved?
\end{quote}

Naturally, this vague statement invites the question ``how is improvement measured?'', or ``improvment in what outcome?''. The question is left deliberately vague in this way because part of this work will be dedicated to teasing apart the different possible interpretations, the importance of taking care when interpreting improvement in an educational context, and the consequences of different interpretations. A solution is not proposed, only alternatives and their consequences, with comments on the stakeholders in each case. In the spirit of being vague however, as educators our interest is always centered on the best interests of our students and this is the general position taken in this work. Of course, determining what is in the best interests of our students, is not always clear.
...
\begin{itemize}
	...
	\item In \refchap{literature} I will provide a indepth discussion of the existing literature, what is known, approaches attempted in the past both in Australia and internationally, and some deeper discussion on some of the particularly relevant related concepts, such as maths anxiety.
...
\section{A Framework to Frame this Work}
\label{sec:framework}
...
The initial phase of the literature review was performed in an iterative process which given a list of references involved reading the list of references and generating a new list repeatedly. In the first iteration, the papers identified where \cite{Nicholas2015}, \cite{Gordon2013}, and \cite{Johnson2016}, and these where found by using search engines, including google scholar and the \gls{uofa} library search with terms such as ``mathematics bridging courses'' in order to identify several recent, directly relevant, key papers to start with. The iterative process then involved reading the current list of references, taking notes and quotations for later use, and compiling a new list of references by:
\begin{itemize}
	...
	\item Additional papers identified by use of seach engines for newly identified key terms, such as ``adult-education'' for example.
\end{itemize}
This iterative process was performed until the same papers kept coming up more and more frequently, which only took approximately three or four iterations. Then, the notes and quotations made while reading these references where reviewed, and synthesised into a coherent discussion, which is presented in \refchap{literature}.

\section{Curriculum Mapping}

The curriculum mapping was performed by first establishing the levels of detail of interest, and terminology for these levels of detail. Specifically, there are two levels of detail at which the curriculum mapping is performed: the topic level, and the key concept level. These are discussed in more detail in \refchap{curriculum}, but very breifly each curriculum is broken up into approximately $12$--$24$ topics, with each of these topics including typlically $6$-$12$ key concepts each. 
...
In this chapter the literature surrounding mathematics bridging courses in Australia and internationally is explored. The literature reviewed here is not restricted strictly to these topics, but also extends into some directly relevant areas, specifically areas such as general perceptions of mathematics, the secondary-tertiary transition, relevant frameworks that have been proposed, and some of the key areas that prevent students from being successful such as maths anxiety. 

TODO: Update intro to literature review.

Addressing the purpose statement of this thesis:
...
assistance so necessary for these students’. NOTE: THis is from MERGA 1993 conference proceedings, tough to find.
		\item For the second question, internationally, bridging mathematics programs have been
shown to be highly effective at resolving skill deficiencies for some students \cite{Kajander2005, Bahr2008}. In a
large US study, \cite[p.442]{Bahr2008} found that ‘remediation has the capacity to fully resolve the
...
Amongt other reasons, this decline in participation in mathematics is a problem in Australia because mathematical skills are essential to just about all the key future industries, and hence the Australian economy. The key economic importance of mathematics is widely acknowledged amongst the academy and industry, but it's importance is often overlooked and difficult to communicate to the wider community because of it's indirect importance through what are perceived to be other fields: engineering, science, etc. all of which require a deep level of mathematical skills, but aren't associated to mathematics in the general populations view. \citeA{Thomas2009} argues that one of the most influentia lfactors in the declining participation in mathematics is the ``community’s perception that mathematics is not useful in the marketplace''. \citeA{Gordon} go on to  emphasise the carry-on effects of negative community perceptions of mathematics leading to highscool students choosing not to participate in higher-level mathematics impacting on not only their success in university, but on whether they continue to study mathematics at all. This obviously has implicatons for mathematics bridging courses at universities, as it is to them that these students will turn should they realise the importance of mathematics later on in life.

Observation, concern surrounding, and research of this decline in mathematics participation in senior highschools are not limited to Australia \cite{Hourigan2007, Hoyles2001}. \cite{Hoyles2001} as well as \cite{Luk2005} further connect this trend to another: the apparent divergence of content (curriculum) between senior secondary and tertiary education. This divergence of curriculum is a point that will be explored extensively in \refchap{curriculum}.


TODO: Re-write the following

\begin{itemize}
	\item Although \cite{Rylands2009} highlighting the lack of appropriate
mathematical background among students at their respective institutes in Australia, this is a trend replicated accross the globe.
...
lighted by \cite{Tariq2002}.
	...
	\item \cite{Croft2009} claims that mathematics skill of students enrolling in STEM degrees is one of the primary predictors for retention.

	\item This is a problem recognised by universities.  \cite{Robinson2003}
 suggests higher level mathematics and science in highschool. Naturally this is beyond the scope of this thesis, and beyond our control and capabilities to implement from a university perspective (aside from the discussion around pre-requisites). Basically though, not going to happen. Hence, as \cite{Kitchen1999}
have highlighted, universities are introducing ``remedial mathematics'' into the first-
year teaching programmes. \cite{Moses2011} suggest ``advanced and targeted preparatory programmes'' (outside of the normal university preparation), i.e. briding programs, and suggest that without such preparation many students will
be more likely to dropout.
	\item Quote from \cite{Johnson2016}:
		\begin{quote}
...
\section{Success in University}

NOTE: Maybe merge this and the next sections together into ``University Entry''

\cite{Kajander2005} Multiple papers reference this study, I should read it and comment. According to \cite{Gordon2013} ``\cite{Kajander2005} found that time spent learning mathemat-
ics in the final years of high school was crucial for students’ success in
university calculus courses.''.

\cite{Nicholas2015b} Impact of HSC mathematics level on success in first year: acheivement band not only level is important and should be taken into account. Bridging courses make a difference, but not as big a difference as HSC maths level.
...
\section{Effects of Conditions on Entry to University Education}

‘perception by students that mathematics is not necessary for engineering or science’ (King and
Cattlin, 2014, p.6)

\citeA{Chubb2012} suggests moving back towards pre-requisites.

\cite{Nicholas2015b} Impact of HSC mathematics level on success in first year: acheivement band not only level is important and should be taken into account. Bridging courses make a difference, but not as big a difference as HSC maths level.

\cite{Gordon2015} ``Assumed Knowledge'' for entry level maths courses at uni.
...
\cite{Gordon2013} abstract:  show that the major reasons for students taking lower
...
maximizing potential ranking for university admission.
...
\section{Secondary-Tertiary Transition as a `Rite-of-Passage'}

\citeA{Clark2008} suggest thinking of the secondary-tertiary mathematics education transition as a 'rite of passage' and to leverage the extensive knowledge of such events from fields such as anthropology and culture studies (relating concepts such as culture shock), to allow us to better understand this transition and design interventions that will be more effective.

They take the seminal work of Arnold van Gennep in describing the separation, liminal, and incorporation phases of a ``life crisis''. 

Taking this viewpoint sheds additional light on the ``mathematics problem'' as not only relating to students transitioning into tertiary education, but their community, their families, and the views held of the importance (or non-importance) of mathematics. 

``there is no such thing as a smooth transition, and that this is not
even necessarily desirable. Shock is inevitable, we must acknowledge it and deal
with it. '' This should be explicitly made clear to students, and that it is ok to be stressed, to find it difficult. 
This matches with the re-evaluation model in maths anxiety.

I'm not sure I agree with this but the rite-of-passage model suggests we abandon imprecise language and descriptions of concepts, in favour of rigorous explicit anguage.

``It is normal to feel
discomfort during a rite of passage but much easier to deal with if this is expected.''

\cite{Gordon2013} shows the value students place on interaction with peers
...
formative in students’ adjustment to and retention in higher education \cite{Peat2001, Trotter2006}. This could be particularly critical for students where family or friends are unfamiliar with the discourse and ways
of learning in the university context \cite{Leese2010}.

\section{non-content benefits of bridging courses}

\cite{Gordon2013}  abstract:  ``students are aware of the value of the
...
--- \cite{Croft2009, MacGillivray2009} could be relevant references for this too.



Quote from \cite{Galligan2008}:
\begin{quote}
In a quantitative approach, \citeA{Klinger2006} investigated attitudes, self-efficacy
...
confirmed that these could be changed. In a later study, \citeA{Klinger2008} repeated
...
\section{TMP SEC: Brief Summaries}
...
\begin{itemize}
	...
	\item Quote from \cite{Galligan2008}:
		\begin{quote}
...
\section{Conclusions/ Take-Away Messages}

\begin{itemize}
	\item Students should be explicitly told that the first semester will be hard. It will be stressful. But that it will be worth it. To expect difficulty, but to embrace it. Several frameworks suggest this, including the rite-of-passage model of \citeA{Clark2008} and the re-evaluation model for maths anxiety from clinical psychology. An emphasis should be placed on not setting expectations that the difficult period will pass quickly, but rather that it is a natural, expected transitionary period. That it will continue to be difficult, but that that is ok.

\cite{Carlson1999} shows that establishing a mindset of persistence tackling complex problems puts students in good stead in ongoing mathematical study. Supporting that establishing this `productive struggle' mindset is effective in acheiving success later on.

	\item TODO: Condense this point.

Various factors have been identified that ease the transition of students
into university and appear to contribute to improved rates of retention. These include activities that help students
